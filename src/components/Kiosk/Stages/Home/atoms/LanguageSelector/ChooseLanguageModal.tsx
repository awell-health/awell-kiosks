import { Dialog, Transition } from '@headlessui/react'
import { useTranslation } from 'next-i18next'
import { Fragment } from 'react'

import { useLanguage } from '../../../../../../hooks/useLanguage'
import { LanguageOption } from './LanguageOption'

export const ChooseLanguageModal = () => {
  const { toggleLanguageModal, isLanguageModalOpen } = useLanguage()
  const { t } = useTranslation()
  return (
    <Transition appear show={isLanguageModalOpen} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-10"
        open={isLanguageModalOpen}
        onClose={() => toggleLanguageModal()}
      >
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-300"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-200"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-black bg-opacity-25" />
        </Transition.Child>

        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0 scale-95"
              enterTo="opacity-100 scale-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100 scale-100"
              leaveTo="opacity-0 scale-95"
            >
              <Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-white p-8 text-left align-middle shadow-xl transition-all">
                <Dialog.Title
                  as="h3"
                  className="text-4xl font-semibold leading-6 text-slate-800 mb-8"
                >
                  {t('choose_your_language')}
                </Dialog.Title>
                <div className="grid gap-4 mx-auto grid-cols-3">
                  <LanguageOption language="en" />
                  <LanguageOption language="fr" />
                  <LanguageOption language="nl" />
                </div>
              </Dialog.Panel>
            </Transition.Child>
          </div>
        </div>
      </Dialog>
    </Transition>
  )
}
